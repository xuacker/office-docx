...
UserName = cf.get("reverse", "user")
Password = cf.get("reverse", "pass")
...
@route('/static/<name:path>', method="GET")
def do_get_static(name):
    return static_file(name, root="%s/static" % currentDir, download=True)
...
def do_post_login():
    ...
    if UserName == username.strip() and Password == password.strip():
        print "ok!"
        s = request.environ.get('beaker.session')
        s["username"] = "admin"
        s.save()
